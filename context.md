...
- **AlphaEarth urban fabric** (overlay "Urban fabric"): `python3 scripts/alphaearth-urban-fabric.py` → `public/data/phuket-urban-fabric.geojson` (k-means land-cover, 2025 embedding). GEE already authenticated.
- **AlphaEarth Risk Twins** (overlay "Risk twins" + Slope Story card row): `python3 scripts/alphaearth-risk-twins.py` → `public/data/phuket-risk-twins.geojson` (890 zones). Embedding cosine-similarity to the 5 high-severity blackspots (64-d L2-normalized → dot product = cosine; threshold 0.85; 800 m self-exclusion). "Where else looks like a known death-corridor." Opacity encodes similarity. Refresh annually.
- **TimesFM per-corridor probabilistic forecast** (Slope Story TONIGHT + Governor brief sparkline): precompute (needs Python 3.10+):
  `uv run --python 3.11 --with "timesfm[torch]" --with requests --with numpy python scripts/timesfm-forecast.py`
  → `public/data/phuket-accident-forecast.json` (48h). TimesFM 2.0 500m with REAL quantile bands (riskLow/riskHigh). Now emits a `corridors` map (Patong/Old Town/Airport north) with rain sensitivity `k=0.18·(meanSlopeDeg/8)` — steep corridors amplify rain more (SRTM slope → forecast). Schema is backward-compatible (island top-level unchanged). Open-Meteo forecast API rate-limits (429) → falls back to archive ERA5 + climatology. Live BigQuery `AI.FORECAST` is the documented future path.
